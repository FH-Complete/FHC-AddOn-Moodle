<?php
$this->phrasen['moodle/sieSindNichtAngemeldet']='You are not logged in. No matching UID was found';
$this->phrasen['moodle/lvidMussUebergebenWerden']='lvid must be provided';
$this->phrasen['moodle/esWurdeKeinStudiensemesterUebergeben']='The study semester was not provided';
$this->phrasen['moodle/esMussMindestensEineLehreinheitMarkiertSein']='At least one teaching unit must be selected';
$this->phrasen['moodle/esWurdeKeineGueltigeIdUebergeben']='The ID was not provided or was invalid';
$this->phrasen['moodle/testkursWurdeErfolgreichAngelegt']='The test course was successfully created';
$this->phrasen['moodle/esExistiertBereitsEinTestkurs']='A test course already exists';
$this->phrasen['moodle/esIstBereitsEinMoodleKursVorhanden']='A Moodle course already exists for the total course';
$this->phrasen['moodle/moodleKursAnlegen']='Create Moodle course';
$this->phrasen['moodle/kursbezeichnung']='Course Name';
$this->phrasen['moodle/gruppenUebernehmen']='Accept groups';
$this->phrasen['moodle/kursAnlegen']='Create course';
$this->phrasen['moodle/vorhandeneMoodleKurse']='Existing Moodle courses for this course';
$this->phrasen['moodle/testkurse']='Test course';
$this->phrasen['moodle/klickenSieHierUmTestkursErstellen']='click here to create a test course';
$this->phrasen['moodle/fehlerBeimAnlegen']='An error occurred when creating the course';
$this->phrasen['moodle/artIstUnbekannt']='unknown "art"';
$this->phrasen['moodle/datenWurdenAktualisiert']='The data has been updated';
$this->phrasen['moodle/fehlerBeiDerLektorenZuordnung']='Error assigning the lecturer';
$this->phrasen['moodle/fehlerBeiDerStudentenZuordnung']='Error assigning the student';
$this->phrasen['moodle/kursUebersicht']='Moodle - Course Overview';
$this->phrasen['moodle/weiterleitung']='You will be forwarded automatically. If you are not forwarded automatically, click <a href="%s">here</a>';
$this->phrasen['moodle/meineKurse']='My Courses';
$this->phrasen['moodle/vorhandeneKurse']='Available Courses';
$this->phrasen['moodle/wartung']='Create course';
$this->phrasen['moodle/bezeichnungMussEingegebenWerden']='A name must be entered';
$this->phrasen['moodle/handbuch']='Manual';
$this->phrasen['moodle/moodle']='Moodle';
$this->phrasen['moodle/kursfuerganzeLV']='one single Moodle course for all groups';
$this->phrasen['moodle/kursfuerLE']='one Moodle course for selected groups';
$this->phrasen['moodle/zusatztextWartung']='';
$this->phrasen['moodle/subTextIcon']='<a href="'.APP_ROOT.'addons/moodle/cis/moodle_wartung.php?lvid=%s&stsem=%s" class="Item">Create course</a>';
$this->phrasen['moodle/success.template.update']='Template successfully saved';
$this->phrasen['moodle/warn.add.multiple']='%d Courses had been created, since the LV Parts have different languages';
$this->phrasen['moodle/warn.add.skip']='There was no Course created for the LV Parts (%s), since the Template has no Source Course for the Language "%s" yet';
$this->phrasen['moodle/warn.moodle.lang.diff']='<b>Attention:</b> There is only one Moodle Course, but the Lehreinheiten have multiple languages!';
$this->phrasen['moodle/error.moodle.wrong']='A Moodle Source Course with this id does not exist';
$this->phrasen['moodle/error.moodle.duplicate']='This Moodle Source Course is already assigned to another Template';
$this->phrasen['moodle/error.template.wrong']='A LV Template with this id does not exist';
$this->phrasen['moodle/error.update.empty']='Nothing to update';
$this->phrasen['moodle/error.moodle.overwrite']='The template is already mapped to a Moodle Source Course. Use the overwrite checkbox to overwrite the assignment';
$this->phrasen['moodle/error.testcourse.invalid']='Fatal: Test Course creation is not possible. Failure in Name creation';
$this->phrasen['moodle/error.sql']='Database Error: %s';
$this->phrasen['moodle/error.sql:']='Unknown Database Error';
$this->phrasen['moodle/quellkurs.title']='Moodle - Source Course administration';
$this->phrasen['moodle/quellkurs.title.assignment']='Assignment administration';
$this->phrasen['moodle/quellkurs.title.assignment.new']='New assignment';
$this->phrasen['moodle/quellkurs.title.assignment.change']='Edit assignment';
$this->phrasen['moodle/quellkurs.title.unassigned']='Templates without Source Course';
$this->phrasen['moodle/quellkurs.form.label.moodle_course']='Moodle Source Course';
$this->phrasen['moodle/quellkurs.form.label.template']='LV Template';
$this->phrasen['moodle/quellkurs.form.label.language']='Language';
$this->phrasen['moodle/quellkurs.form.label.overwrite']='Overwrite';
$this->phrasen['moodle/quellkurs.form.btn.delete']='Delete assignment';
$this->phrasen['moodle/model.lv.id']='Id';
$this->phrasen['moodle/model.lv.bezeichnung']='Title';
$this->phrasen['moodle/form.text.or']='-or-';
$this->phrasen['moodle/form.btn.show']='show';
$this->phrasen['moodle/form.btn.save']='save';
$this->phrasen['moodle/form.btn.back']='back';
$this->phrasen['moodle/form.btn.repair']='repair';
?>
