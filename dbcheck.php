...
if($result = $db->db_query("SELECT 1 FROM system.tbl_berechtigung WHERE berechtigung_kurzbz='addon/moodle_quellkurs'"))
{
	if($db->db_num_rows($result)==0)
	{
		$qry="INSERT INTO system.tbl_berechtigung(berechtigung_kurzbz, beschreibung) VALUES('addon/moodle_quellkurs','Addon Moodle Quellkursverknüpfung');";

		if(!$db->db_query($qry))
			echo '<strong>'.$db->db_last_error().'</strong><br>';
		else
			echo ' neue Berechtigung addon/moodle_quellkurs hinzugefuegt!<br>';
	}
}
